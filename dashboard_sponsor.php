<?php
// Autor: Julian Ploch
// Beschreibung: Dashboard für den Sponsor nach dem Login

// Session-Sicherheit: HttpOnly, Strict-Mode und SameSite verhindern XSS- und CSRF-Angriffe
ini_set('session.cookie_httponly', 1);
ini_set('session.use_strict_mode', 1);
ini_set('session.cookie_samesite', 'Strict');
session_start();

// Datenbankverbindung und Hilfsfunktionen laden
require_once __DIR__ . '/process.php';

// Zugriffskontrolle: Nur eingeloggte Sponsoren dürfen diese Seite sehen
...
// Alle zukünftigen Rennen aus der Datenbank-View laden
function getZukuenftigeRennen(PDO $db): array {
    return $db->query("SELECT * FROM v_ZukuenftigeRennen")->fetchAll(PDO::FETCH_ASSOC);
}

// Gesamtbudget und Restbudget des Sponsors laden; Fallback auf 0 falls kein Eintrag
...
// Budgetverteilung des Sponsors über alle gesponserten Rennen laden
...
// Renn-IDs laden, denen der Sponsor bereits ein Budget zugewiesen hat
...
// Budgetzuweisung speichern; gibt "ok" oder eine Fehlermeldung zurück
function budgetZuweisen(PDO $db, int $sponsorID, int $rennID, float $betrag): string {

    // Doppelzuweisung verhindern: ein Sponsor kann pro Rennen nur einmal sponsern
    ...
    // Betrag darf das verfügbare Restbudget nicht überschreiten
    ...
    // Eintrag speichern; Trigger trg_RestBudgetNachEintrag zieht den Betrag automatisch ab
...
// POST-Verarbeitung mit PRG-Muster: verhindert doppelte Übermittlung bei Seitenaktualisierung
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['renn_id'], $_POST['betrag'])) {
    // Komma als Dezimaltrennzeichen abfangen (deutsche Tastatureingabe)
...
// Flash-Messages aus Session lesen und sofort löschen
...
// Bereits gesponserte Rennen aus der Auswahlliste herausfiltern
